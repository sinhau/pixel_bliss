...
class TestAsyncPromptGeneration:
    ...
    @patch('pixelbliss.alerts.discord_select.ask_user_to_select_raw')
    @pytest.mark.asyncio
    async def test_post_once_discord_human_selection_timeout(self, mock_discord_select, mock_sequential, mock_variants, mock_base, mock_category, mock_config):
        """Test post_once with Discord human selection timeout (pipeline ends early)."""
        # Setup config with Discord enabled
        mock_cfg = Mock()
        mock_cfg.categories = ["test"]
        mock_cfg.image_generation.async_enabled = False
